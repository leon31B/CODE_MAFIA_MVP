...
const BOT_NAMES = [
  'Alex_Dev', 'Sam_Cyber', 'Morgan_Code', 'Taylor_Stack',
  'Jordan_Byte', 'Casey_Debug', 'Riley_Logic', 'Devin_Script',
  'Avery_Syntax', 'Quinn_Null'
];
...
export function generateBotPlayers(countNeeded: number, startIndex: number = 1): Player[] {
  ...
  for (let i = 0; i < countNeeded; i++) {
    ...
    bots.push({
      id: `bot-${Date.now()}-${i}`,
      displayName: `${BOT_NAMES[nameIndex]} (AI)`,
      isAlive: true,
      isHost: false,
      isBot: true,
      isReady: true,
      ...
      stats: {
        ...
        testsRun: 0,
        votesCast: 0,
        suspicionScore: Math.floor(Math.random() * 30)
      }
    });
  }
  return bots;
...
export function decideBotVote(
  ...
  if (bot.role === 'MAFIA') {
    ...
    // Developer targets player with highest suspicion or random
